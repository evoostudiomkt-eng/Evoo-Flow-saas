...
const STAGES = [
  { id: 'new', label: 'Prospecção', color: 'bg-blue-600', light: 'bg-blue-50', text: 'text-blue-700' },
  { id: 'contacted', label: 'Qualificação', color: 'bg-amber-500', light: 'bg-amber-50', text: 'text-amber-700' },
  { id: 'proposal', label: 'Proposta', color: 'bg-purple-600', light: 'bg-purple-50', text: 'text-purple-700' },
  { id: 'won', label: 'Ganho', color: 'bg-green-600', light: 'bg-green-50', text: 'text-green-700' },
  { id: 'lost', label: 'Perdido', color: 'bg-red-600', light: 'bg-red-50', text: 'text-red-700' },
] as const;
...
export default function Leads({ profile, isDemoMode }: LeadsProps) {
  ...
  const [newLead, setNewLead] = useState({ name: '', email: '', company: '', source: 'LinkBio' });
  ...
  return (
    <div className="space-y-8" id="leads-pipeline">
       <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold text-gray-900 font-sans tracking-tight">Pipeline de Vendas</h2>
          <p className="text-gray-500 mt-1">Transforme oportunidades em resultados reais.</p>
        </div>
        <div className="flex flex-wrap gap-3">
          <button 
            ...
            className={cn(
              ...
              copied ? "bg-green-600 text-white shadow-green-100" : "bg-white text-gray-700 border border-gray-100 hover:border-blue-200"
            )}
          >
      ...
      <div className="flex gap-6 overflow-x-auto pb-10 h-[calc(100vh-14rem)] custom-scrollbar" id="leads-grid-container">
         {STAGES.map((stage) => {
           const stageLeads = leads.filter(l => l.status === stage.id);
           return (
            <div key={stage.id} className="min-w-[320px] w-80 flex flex-col space-y-5" id={`stage-${stage.id}`}>
              <div className="flex items-center justify-between px-3">
                 <div className="flex items-center space-x-2">
                    <div className={cn("w-2 h-2 rounded-full", stage.color)}></div>
                    <span className="text-[11px] font-extrabold text-gray-400 uppercase tracking-[0.2em]">{stage.label}</span>
                 </div>
                 <span className="bg-gray-100/80 text-gray-400 text-[10px] font-bold px-2 py-0.5 rounded-full">{stageLeads.length}</span>
              </div>
              ...
              <div className="flex-1 space-y-4 bg-gray-50/50 p-3 rounded-[2rem] border border-gray-100 overflow-y-auto custom-scrollbar">
                 {stageLeads.map(lead => (
                   ...
                   >
                      <div className="flex justify-between items-start">
                         <div className="flex items-center space-x-3">
                            ...
                            <div>
                               ...
                               <p className="text-[10px] font-bold text-gray-400 uppercase mt-0.5">{lead.source}</p>
                            </div>
                         </div>
                      </div>
                      ...
                      <div className="flex items-center justify-end space-x-1 pt-2">
                         {STAGES.filter(s => s.id !== lead.status).slice(0, 3).map(next => (
                           <button 
                             key={next.id}
                 ...
                 {stageLeads.length === 0 && (
                    <div className="py-12 flex flex-col items-center justify-center text-gray-300 opacity-50">
                       ...
                       <p className="text-[10px] font-bold uppercase tracking-widest text-center">Vazio</p>
                    </div>
                 )}
              </div>
            </div>
      ...
      <AnimatePresence>
        {isAdding && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4">
            ...
            >
               <h3 className="text-2xl font-bold text-gray-900 mb-8 font-sans">Cadastrar Lead</h3>
               <form onSubmit={handleCreate} className="space-y-5">
                  ...
                  <div className="space-y-1">
                    <label className="block text-sm font-bold text-gray-700 ml-1">Instituição</label>
                    <input 
                      type="text" required
                      className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 transition-all"
                  ...
                  <div className="flex space-x-3 pt-6">
                    ...
                    <button type="submit" className="flex-1 py-4 bg-blue-600 text-white rounded-2xl font-bold shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all">Salvar</button>
                  </div>
               </form>
            </motion.div>
          </div>
...
const UserProfileInitials = ({ name }: { name: string }) => {
  ...
  return <span className="text-[11px] font-bold text-blue-600">{initials}</span>;
};
